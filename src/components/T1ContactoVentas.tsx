...
const VALUE_PROPS = [
  { t: "A tu medida", d: "Configuramos T1 según la operación, los volúmenes y los flujos de tu empresa." },
  { t: "Mejores costos y comisiones", d: "Tarifas negociadas por volumen en pagos y envíos, a la escala de tu negocio." },
  { t: "API de alta capacidad", d: "Hasta 10× de ancho de banda para integrar T1 con tus sistemas sin cuellos de botella." },
  { t: "Asesoría y soporte prioritario", d: "Un equipo dedicado que te acompaña en la integración y la operación diaria." },
];
  ...
  return (
    <main className="min-h-screen bg-black">
      ...
      <section className="relative overflow-hidden px-5 pb-24 pt-[130px] tablet:px-10 tablet:pt-[150px]" style={{ background: HERO_BG }}>
        ...
        <div className="relative z-10 mx-auto grid max-w-[var(--max-w)] grid-cols-1 gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:gap-16">
          {/* Left — pitch */}
          <div className="lg:pt-6">
            ...
            <h1 className="font-sora text-[34px] font-light text-white tablet:text-[52px]" style={{ letterSpacing: "-0.03em", lineHeight: 1.08, marginTop: 20, maxWidth: 540 }}>
            ...
            <p className="font-inter text-[16px] font-light text-white/70 tablet:text-[18px]" style={{ lineHeight: 1.55, marginTop: 20, maxWidth: 480 }}>
              Cuéntanos de tu operación y un experto arma un plan de vender, cobrar y enviar a la escala de tu negocio.
            </p>

            <ul className="mt-10 flex flex-col gap-6" style={{ maxWidth: 460 }}>
              {VALUE_PROPS.map((v) => (
                <li key={v.t} className="flex items-start gap-3.5">
                  <span className="mt-0.5 flex h-[26px] w-[26px] shrink-0 items-center justify-center rounded-full bg-[rgba(219,59,43,0.14)]">
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M5 12L10 17L19 7" stroke="#DB3B2B" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" /></svg>
                  </span>
                  <span>
                    <span className="block font-sora text-[17px] font-normal text-white">{v.t}</span>
                    <span className="mt-1 block font-inter text-[14px] font-light leading-relaxed text-white/55">{v.d}</span>
                  </span>
                </li>
              ))}
            </ul>
          ...
          <div className="rounded-[24px] border border-white/[0.10] bg-white/[0.03] p-6 tablet:p-8">
            {sent ? (
              ...
              >
                ...
                <label className="block">
                  <span className={LABEL}>Necesidades especiales<span className="text-white/35"> (opcional)</span></span>
                  <textarea rows={3} placeholder="Cuéntanos qué necesitas resolver" className="w-full resize-none rounded-[12px] bg-[#1D1D1D] px-4 py-3 font-inter text-[15px] text-white outline-none transition-shadow placeholder:text-[#8A8A8A] focus:ring-1 focus:ring-white/25" />
                </label>
