        ...
        <div className="grid grid-cols-1 gap-4 tablet:grid-cols-3 tablet:gap-5">
          {CARDS.map((c) => (
            <div key={c.title} className="flex flex-col rounded-[20px] border border-white/[0.08] bg-[#1A1A1D] p-7 tablet:p-8">
              ...
              <a href={c.href} data-cta-text={c.cta} data-cta-destination={c.href} data-cta-section="simplifica" className="mt-auto inline-flex items-center gap-1.5 font-inter text-[14px] font-semibold text-white no-underline transition-colors hover:text-white/70">
                {c.cta}
