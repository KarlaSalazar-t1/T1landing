            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              {SOLUTION_CHIPS.map((c) => (
                <a
                  key={c.label}
                  href={c.href}
                  className="group inline-flex w-[220px] items-center gap-3 rounded-[14px] border border-white/[0.1] bg-white/[0.06] py-3.5 pl-5 pr-4 no-underline transition-all duration-150 hover:border-white/20 hover:bg-white/[0.12]"
                >
                  <span className="flex items-center text-white/90">{c.icon}</span>
                  <span className="font-inter text-[15px] font-medium text-white/85 tablet:text-[16px]">{c.label}</span>
