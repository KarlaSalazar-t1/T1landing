...
function BentoCard({ span, dark = false, style, children }: { span: string; dark?: boolean; style?: React.CSSProperties; children: React.ReactNode }) {
  return (
    <div
      data-stagger
      className={`group relative overflow-hidden rounded-[20px] border p-6 transition-all duration-300 hover:-translate-y-1 ${span} ${dark ? "border-white/[0.08] bg-[#0E0E0E] hover:shadow-[0_24px_60px_rgba(0,0,0,0.5)]" : "border-black/[0.07] bg-white hover:shadow-[0_24px_60px_rgba(0,0,0,0.10)]"}`}
      style={style}
    >
      {children}
    </div>
...
/* WhatsApp-style chat con "escribiendo…" y palomitas enviado → entregado → leído (autoplay) */
function WAticks({ state }: { state: string }) {
  if (state === "hidden" || state === "typing") return null;
  const read = state === "read";
  const color = read ? "#53BDEB" : "rgba(0,0,0,0.35)";
  const single = state === "sent";
  return (
    <span className="ml-1 inline-flex shrink-0 self-end" style={{ marginBottom: 1 }}>
      <svg width={single ? 12 : 17} height="11" viewBox="0 0 17 11" fill="none">
        <path d="M1 6L4 9L9.5 2.5" stroke={color} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
        {!single && <path d="M7 9L12.5 2.5" stroke={color} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />}
      </svg>
    </span>
  );
}

/* Demoras — revela primero la incidencia y luego el seguimiento (fade in, autoplay en loop) */
function DemorasFeed() {
  // step: 0 reset/oculto · 1 incidencia · 2 incidencia + seguimiento · 3-4 hold
  const [step, setStep] = useState(0);
  useEffect(() => {
    const id = setInterval(() => setStep((p) => (p + 1) % 5), 1200);
  ...
  return (
    <div style={{ minHeight: 96 }}>
      {step >= 1 && (
        <div
          className="flex items-center gap-2.5 rounded-[12px] bg-[rgba(245,158,11,0.10)] px-3 py-3"
          style={{ fontFamily: MANROPE, marginBottom: 10, animation: "rastreoReveal 0.5s cubic-bezier(0.16,1,0.3,1) both" }}
        >
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="shrink-0"><path d="M12 9v4M12 17h.01M10.3 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.7 3.86a2 2 0 0 0-3.42 0z" stroke="#FBBF24" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" /></svg>
          <div className="min-w-0">
            <p className="truncate text-[12px] font-semibold text-white">Incidencia · #PX340928177</p>
            <p className="truncate text-[11px] text-white/55">Sin movimiento por 48 h</p>
          </div>
        </div>
      )}
      {step >= 2 && (
        <div className="flex items-center gap-2" style={{ fontFamily: MANROPE, animation: "rastreoReveal 0.5s cubic-bezier(0.16,1,0.3,1) both" }}>
          <span className="flex h-[16px] w-[16px] items-center justify-center rounded-full bg-[#16A34A]"><svg width="9" height="9" viewBox="0 0 16 16" fill="none"><path d="M3 8L6.5 11.5L13 4.5" stroke="#FFFFFF" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" /></svg></span>
          <span className="text-[11px] text-white/70">Equipo T1 dando seguimiento</span>
        </div>
      )}
    </div>
  );
...
function WhatsAppChat() {
  const MSGS = ["Va en camino, llega mañana ✨", "¡Tu paquete está en reparto hoy! 🚚"];
  const FRAMES: { typing: number; st: string[] }[] = [
    { typing: 0, st: ["hidden", "hidden"] },
    { typing: -1, st: ["sent", "hidden"] },
    { typing: -1, st: ["delivered", "hidden"] },
    { typing: 1, st: ["read", "hidden"] },
    { typing: -1, st: ["read", "sent"] },
    { typing: -1, st: ["read", "delivered"] },
    { typing: -1, st: ["read", "read"] },
    { typing: -1, st: ["read", "read"] },
  ];
  const [f, setF] = useState(0);
  useEffect(() => {
    const id = setInterval(() => setF((p) => (p + 1) % FRAMES.length), 950);
    return () => clearInterval(id);
  }, [FRAMES.length]);

  const frame = FRAMES[f];
  const allRead = frame.st.every((s) => s === "read");

  return (
    <div className="flex flex-col gap-2" style={{ fontFamily: MANROPE, minHeight: 132 }}>
      {MSGS.map((m, i) => {
        if (frame.typing === i) {
          return (
            <div key={`t${i}`} className="flex items-center gap-1 self-start rounded-[12px] rounded-bl-[4px] bg-[#E8F8EE] px-3.5 py-3" style={{ animation: "rastreoReveal 0.3s ease both" }}>
              {[0, 1, 2].map((d) => (
                <span key={d} className="wa-dot h-[6px] w-[6px] rounded-full bg-black/40" style={{ animationDelay: `${d * 0.18}s` }} />
              ))}
            </div>
          );
        }
        if (frame.st[i] === "hidden") return null;
        return (
          <div key={i} className="flex items-end self-start rounded-[12px] rounded-bl-[4px] bg-[#E8F8EE] px-3 py-2" style={{ maxWidth: "92%", animation: "rastreoReveal 0.4s cubic-bezier(0.16,1,0.3,1) both" }}>
            <p className="text-[12px] text-black/80">{m}</p>
            <WAticks state={frame.st[i]} />
          </div>
        );
      })}
      {allRead && (
        <div className="mt-1 flex items-center gap-2" style={{ animation: "rastreoReveal 0.4s ease both" }}>
          <svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M3 8L6.5 11.5L13 4.5" stroke="#16A34A" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
          <span className="text-[11px] text-black/55">Enviados en automático</span>
        </div>
      )}
    </div>
  );
